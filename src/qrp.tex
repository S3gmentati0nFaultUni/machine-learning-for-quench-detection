...
The performance of the model, averaged over the outer \cv\ folds, is reported in \Cref{tbl:an-2-12-perf},
...
\begin{figure}[!ht]
	...
	\caption{Performance comparison of the best decision tree for each attribute. The
		...
		\Cref{chp:problem}.} \label{fig:best-dts}
\end{figure}
...
complex and the ensemble is not too big, \rfs\ are still quite easy to interpret, while
performing usually at least as good, if not better than the worst tree in the ensemble. Since a
single \rf\ contains more \dts\, they can extrapolate information from complicated datasets containing different sub-views (e.g., a dataset containing a sub-view for each of \an, \bn and \cnmod).
...
\begin{figure}[!ht]
	...
	\caption{Performance for the $4$ highest performing forests, independently from which
		...
		\Cref{chp:problem}.} \label{fig:best-rfs}
\end{figure}
...
The best \rf\ we just found contains $5$ trees, each has a maximum depth of $4$ and the number of
...
in \Cref{chp:ml}, and considers each sample as a vector in Euclidean space, and finds a decision
surface separating the vectors corresponding to two classes. This is achieved by maximizing the
\emph{margin} we introduced in \Cref{chp:ml}, intended as the minimal distance between the decision surface and the data images through a nonlinear mapping. The related optimization process exploits the \emph{kernel trick} to automatically select this mapping within some prefixed families.
...
To close the chapter dedicated to \qrp\ we will discuss Tree Aggregators (\tas from here on).
...
know how well they perform from the analysis of the performance obtained in the outer \cv\
folds, we can create an ensemble of the best tree for every attribute, and then we can aggregate the
...
single classifiers on the reduced dataset using a $5$-fold \cv, and finally we aggregate all the predictions via
...
the reduced dataset (the $250$ samples at our disposal). This choice forced us to repeat a
training procedure whenever we wanted to test the performance of models in different configurations,
since the only non-used data available to us were the ones stored in the safe, which was not used
until the last possible moment to avoid biased model choices.
...
We tested the performance of the \ta\ model also using a different sub-view for \cnmod, which performed
better as a tree, but it yielded worse performance when put in the ensemble. For further
testing, we picked the best \ta\ (the one built on \an, \bn\ and \cnmod) and we compared its
performance against the best \svc\ and the best \rf.\Cref{fig:ta-rf-svc-comparison} compares the
performance of the models, the performance of \tas\ are closer to \svc\ than they are to \rf, the only case in which \rf\ is better than \ta\ is the ROC-AUC score.

Since both \ta\ and \rf\ predictions are aggregated via majority voting we could explain this odd
behavior by highlighting that the number of trees contained in the \rf\ is larger than the number of
trees contained in the \ta\ therefore decisions could be slightly more biased: in the first case we need
...
The best aggregator was trained on \an, \bn\ and \cnmod, the model contains $3$ trees: The one built
...
Finally, we evaluated the best TA on the blind test set \db, obtaining only an incorrect
...
results of a final blind-test. In light of all the experiments that we have done we have good reason
to believe that the \ta\ is better than the alternatives because of the very high performance and
ease of interpretability.

If we had to declare an alternative we would indicate the Decision tree as the second best model
simply because of the extreme simplicity of the architecture, only a couple of seconds are necessary
to understand whether a sample is quench or non-quench and it gives an extremely simple description
of the problem. We chose it over random forests because they involve many different harmonics from
different attributes making them more powerful but also less understandable than both \tas\ and
\dts.
