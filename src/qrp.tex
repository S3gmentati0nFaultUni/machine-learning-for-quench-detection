...
its trees is $4$ and the maximum number of internal nodes is $5$, but the majority of the estimators
contain only $4$ nodes. The most important harmonics in the structure are $2$ and $5$, for \cnmod,
$6$ and $10$ for \phin.
...
\rfs, we first aggregated the best decision tree for every table considered, then we retrained the
...
\subsubsection{Final performance analysis}
