...
Let us consider a pair $(\Omega, \mathcal{B})$; the first is called \emph{sample space}, the second is
a $\sigma$-algebra, which is a collection of subsets of $\Omega$ having the following properties:
\begin{itemize}
...
\end{itemize}
The smallest possible $\sigma$-algebra for $\Omega$ is $\emptyset$, the largest would be its power-set
$\mathcal{P}(\Omega)$.

A probability space is a triple $(\Omega, \mathcal{B}, Pr)$ where: $\Omega$ is a sample space,
$\mathcal{B}$ is a $\sigma$-algebra as we just defined it and $Pr$ is a probability measure
assigning a real value to every number $Pr: \mathcal{B} \rightarrow \R$.
...
\mathcal{B}_A)$, and a measurable function for $(\Omega, \mathcal{B})$, taking values in $(A,
\mathcal{B})$, is a mapping $f: \Omega \rightarrow A$ such that: if $F \in \mathcal{B}_A \implies f^
{-1}(F) = \{\omega: f(\omega) \in F\} \in \mathcal{B}$. We rename a measurable function a random
variable if $A$ is $\R$ and $\mathcal{B}$ is a Borel field~\footnote{
	A Borel field is the smallest $\sigma$-algebra built on $\Omega$ containing every open subset of $\Omega$.
