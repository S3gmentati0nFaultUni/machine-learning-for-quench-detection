...
\Cref{chp:problem}): \emph{Entropy} and \emph{Gini index}.
...
probability space $\Omega$. Every partition is chosen based on the outcome of the measurement of the random variable $f$. In the case of a discrete random variable we can define the information entropy as shown in \Cref{eq:information-entropy}
\begin{equation}
...
As shown in \Cref{eq:succ-probability}, if we assume the learners to be independent of each other,
...
\Cref{eq:bagging-limit} is the result of considering the extreme case, $n \rightarrow \infty$, and
...
\begin{equation}
	...
	\end{aligned}
\end{equation}
The hyperplane that we are looking for can be modelled as shown in \Cref{eq:of}, the second
...
\end{equation}
Since \Cref{eq:primal} is an optimization problem with inequality constraints, it must satisfy
Karush-Kuhn-Tucker necessary conditions, first introduced in~\cite{kkt1951}\footnote{For a small
ex-cursus on Karush-Kuhn-Tucker conditions, refer to \Cref{app:kkt}.}.
\begin{equation}
...
\end{equation}
In \Cref{eq:sm-primal} $C > 0$ is a regularization constant and $\ell_{0 / 1}$ is the $0/1$
...
\Cref{eq:cluster-diam} defines the diameter of a cluster, which is the maximum distance between any
