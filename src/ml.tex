...
\begin{equation}
	\label{eq:of}
	f(\vec{x}) = \vec{w}^\top\vec{x} + b = \left(\sum_{i =
		1}^m\alpha_iy_i\vec{x}_i\right)^\top\vec{x} + b
\end{equation}
...
linearly separable, but it's known that such a precondition is extremely strong, since most of the
problems treated in the real world are not actually linearly separable. To address the issue the
