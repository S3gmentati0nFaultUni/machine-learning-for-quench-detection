...
\Cref{fig:clustering-an} plots the result of a $8$ cluster $k$-means clustering run. As we can see,
...
\end{figure}
In \Cref{fig:cluster-score-an} we plot the clustering-specific performance metrics we introduced in
\Cref{chp:ml}. We can draw two different conclusions by looking at the plot:
\begin{inparaenum}[(i)]
