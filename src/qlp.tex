...
have high expectations for such models since they would have to predict $4$ different bits, instead of just one, making the problem much more difficult than the original one.

As a matter of fact, trees, which have always been quite easy to interpret, in the case of \qlp\
became much more complicated. Even if they always had maximum depth between $5$ and $10$, the number
of total nodes was quite high, and the accuracy was close to $60 - 70\%$. This rough performance
number was obtained by micro averaging the accuracy of the classifier for every class, in other words, the resulting metric is the average of the metric computed on each class weighed on the cardinality of the class.

Since the extension of \dts\ did not suite our expectations, we chose to change approach. Our new
idea was simply to turn \qlp\ into a binary classification problem.

To operate this transformation we had to first define a variation of \qrp\ defined on single coils
instead of the whole quadrupole. This variation, which we will call \qrp-$i$, is capable of predicting,
given the harmonic decomposition of magnetic field, whether a certain coil quenched $1$ or not $0$.

The $4$ bit label associated to each sample for \qlp, can be decomposed into the $4$ different bits,
stating whether one of the $4$ coils has quenched or not. Being able to predict this vector means
solving \qrp-$i$ for $i \in \{0, 1, 2, 3\}$.

We solved a variation of \qrp\ and not \emph{the actual} \qrp, since the function we found with
...
an important one. So much so that, if we tried to reuse the best models we found for \qrp, to solve
\qrp-$i$ (for any $i \in \{0, 1, 2, 3\}$), we wouldn't necessarily get the expected performance metrics.
...
concentrate on the performance of the aggregate. \Cref{eq:hamming} shows the technique we used to
compute the performance for the aggregate model, which contains one classifier per coil, we
denominated the metric 'Hamming score' ($\hs$ in the following), and it computes the Hamming distance between the expected and the predicted vectors, and then normalizes it.
\begin{equation}
	\label{eq:hamming}
...
in \Cref{tbl:tree-description}, all the trees described have a depth of maximum $5$, the number
of internal nodes and leaves is always acceptable. While the trees built on \an\ remain on the
smaller side, the trees used for coil $1$ and $3$ are larger (with the one built on \cnmod\ being
the largest at $15$ total nodes).
\begin{figure}[!ht]
	\centering
...
did when testing \tas\ for \qrp: A run of $5$-fold \cv, for each sample we computed $\hs$ and then
plotted the point. On average the ensemble makes $2$ errors or less, on each sample, since the
...
performance on average $0.966$ and a standard deviation $0.086$ of (this difference is probably
due to the lower amount of data we are averaging on), the trained ensemble makes $4$ single errors
on the $116$ total labels ($4$ binary labels for $29$ samples) that it needs to predict.
\begin{figure}[!ht]
	\centering
...
In \Cref{tbl:dt-errors} calculated the number of errors done by every model on all the testing folds
to compare it better to alternatives. It's very interesting that the highest amount of errors came
from the model built on coil $1$, which technically was the one that provided the highest possible
accuracy.
\begin{table}[!ht]
	...
	\begin{tabular}{ccccc}
		...
		\\
		\midrule
		\textsc{Fold 0}         & $2$           & $4$           & $1$            & $3$            \\
		\textsc{Fold 1}         & $1$		& $2$		& $1$            & $2$		  \\
		\textsc{Fold 2}		& $2$           & $3$		& $0$            & $2$            \\
		\textsc{Fold 3}         & $9$           & $5$ 		& $1$            & $2$            \\
		\textsc{Fold 4}         & $8$		& $0$           & $1$            & $3$            \\
		\midrule
		\textsc{total errors}	& $22$		& $14$		& $4$		& $12$
		\\
		\bottomrule
	\end{tabular}
\end{table}
...
the ensemble better because, while we have been saying that the model is an aggregate of trees, we
only inserted the bits in a prediction array. This entails that the stronger model is not making up
for the errors made by the weaker classifiers (which is actually one of the reasons why \rfs\ are
more powerful than single trees, cfr. \Cref{chp:ml}).
...
off from the numbers obtained in the case of \dts\ (cfr. \Cref{fig:bdts-qlp}). The weakest
learner, in this case as well, was the one built on coil $3$.
\begin{figure}
	\centering
...
\Cref{tbl:forest-description} contains basic information regarding the $4$ forests we just
identified as the best ones. The depth, Number of internal nodes and number of leaves has been
averaged on all the trees to give an idea of what the average tree would look like. In the table was
also included the $5$ most important harmonics for each forest, it's interesting that in the case of
coil $0$, despite it using $3$ different datasets, predilects the use of \an\ harmonics to perform
the splits. Clearly the easiest configuration is the one built for coil $0$, the ones for coils $1, 2$ and $3$ are more complex and definitely reach a point where, in our opinion, the model is explainable only on paper.
\begin{table}[!ht]
...
In \Cref{fig:svc-hs} we plotted $\hs$ for the $5$ cross validation folds, as we did for \dts, in
...
is $0.968$ and its standard deviation is $0.013$.
\begin{table}[!ht]
	...
	\begin{tabular}{ccccc}
		...
		\\
		\midrule
		\textsc{Fold 0}         & $0$           & $2$           & $2$            & $2$            \\
		\textsc{Fold 1}         & $0$		& $2$		& $1$            & $1$		  \\
		\textsc{Fold 2}		& $1$           & $5$		& $1$            & $4$            \\
		\textsc{Fold 3}         & $0$           & $2$ 		& $2$            & $3$            \\
		\textsc{Fold 4}         & $0$		& $1$           & $1$            & $2$            \\
		\midrule
		\textsc{total errors}	& $1$		& $12$		& $7$		& $12$
		\\
		\bottomrule
	\end{tabular}
\end{table}

On the blind test set the \svc\ did $4$ errors, and had a final $\hs$ of average $0.966$ and
standard deviation $0.086$. The performance for \svcs\ and \dts\ are not that far off, meaning that
we might be hitting a performance ceiling.
